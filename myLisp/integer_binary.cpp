...
Ptr IntegerBinaryFunction::apply_binary(const Fractional &first, const Fractional &second, State &state) {
	static BigInt one(1);
	if (first.denominator() != one) return state.creator()->new_error("first argument must be integer");
	if (second.denominator() != one) return state.creator()->new_error("second argument must be integer");
