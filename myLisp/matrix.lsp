...
(defn ("mtx-transpose" "ms")
    (defn ("add-2-row" "vs" "rs")
        (if (null? vs)
            ()
            (cons
                (add-lists (car rs) (list (car vs)))
                (add-2-row (cdr vs) (cdr rs))
            )
        )
    )
    (defn ("helper" "cols" "rows")
        (if (null? cols)
            rows
            (helper (cdr cols) (add-2-row (car cols) rows))
        )
    )
    (helper ms ())
)
...
(defn ("mtx-mult" "as" "bs")
    ...
    (assert (= (mtx-mult ((1 2) (3 4)) ((5) (6))) ((17) (39))) "matrix times vector")
